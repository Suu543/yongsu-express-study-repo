...
```javascript
function getGitHubUser(username, password, callback) {
  setTimeout(() => {
    console.log("사용자 정보를 DB에서 읽고있습니다.");
...
function getGitHubRepo(username, callback) {
  setTimeout(() => {
    console.log(
      "해당 사용자의 정보를 받아왔고, 해당 사용자의 Repositoriy를 받아오겠습니다."
    );
    callback([`${username} 사용자가 promise공부 repo를 읽어옵니다`]);
  }, 100);
}

function commitGitHubRepo(repo, callback) {
  setTimeout(() => {
    console.log("커밋할 수 있는 Repository를 받아왔습니다..");
    callback([`${repo}에 커밋 하겠습니다 - promise 공부`]);
  });
}

console.log("시작");

getGitHubUser("cozy5266", "1234567890", (obj) => {
  getGitHubRepo(obj.username, () => {
    commitGitHubRepo("promise repo", (commits) => {
...
```javascript
function getGitHubUser(username, password) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      console.log("사용자 정보를 DB에서 읽고있습니다.");

      if (username == "cozy5027") {
...
function getGitHubRepo(username) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      resolve([`${username} 사용자가 promise공부 repo를 읽어옵니다`]);
    }, 100);
  });
}

function commitGitHubRepo(repo) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      console.log("커밋할 수 있는 Repository를 받아왔습니다..");
      resolve([`${repo}에 커밋 하겠습니다 - promise 공부`]);
    });
  });
}

getGitHubUser("cozy5027", "1234567890")
  .then((사용자정보) => getGitHubRepo(사용자정보.username))
  .then((저장소) => commitGitHubRepo(저장소[0]))
  .then((커밋결과) => console.log(커밋결과[0]))
  .catch((오류코드) => console.log(오류코드));
```
...
async function run() {
  try {
    let 사용자정보 = await getGitHubUser("cozy5027", "1234567890");
    let 저장소 = await getGitHubRepo(사용자정보.username);
    let 커밋결과 = await commitGitHubRepo(저장소[0]);
    console.log(커밋결과[0]);
  } catch (error) {
...
3. `VSCode`에서 `run()`에 마우스를 대면 `return type`이 `void`라 나온다. 이것은 `promise`를 이미 `return` 하고 난 뒤 어떠한 것도 `return` 할 것이 없기 때문이다. 만일 `return` 값이 `promise`가 아닌 경우 `void`가 아닌 `undefined`이 출력됩니다.
